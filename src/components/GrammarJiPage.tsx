    ...
    <div className="grammar-page">
      <div className="grammar-page__hero">
        ...
        <div className="grammar-page__hero-body">
          ...
          <div className="grammar-page__hero-meaning">— {language === 'ru' ? 'сколько? / как много?' : 'necha? / qancha?'} —</div>
        </div>
      </div>
      ...
      <div className="grammar-page__content">
            ...
            <div className="grammar-block">
              ...
              <div style={{ display: 'flex', gap: 12, marginBottom: 14 }}>
                <div style={{ flex: 1, textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 11, color: COLOR }}>{language === 'ru' ? '3-й тон' : '3-ton'}</div>
                </div>
                ...
                <div style={{ flex: 1, textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 11, color: COLOR_DARK }}>{language === 'ru' ? '1-й + 3-й тон' : '1-ton + 3-ton'}</div>
                </div>
              </div>
              <div style={{ fontSize: 12, color: '#666', lineHeight: 1.8 }}>
                <b style={{ color: '#1a1a2e' }}>{language === 'ru' ? 'Значение' : 'Ma\'nosi'}:</b>{' '}
                {language === 'ru' ? 'сколько? как много? какое количество?' : 'necha? qancha? nechta?'}<br />
                <b style={{ color: '#1a1a2e' }}>{language === 'ru' ? 'Черт 几' : '几 chiziqlar'}:</b> 2{' '}
                &nbsp;|&nbsp;{' '}
                <b style={{ color: '#1a1a2e' }}>{language === 'ru' ? 'Черт 多少' : '多少 chiziqlar'}:</b> 6 + 4<br />
                <b style={{ color: '#1a1a2e' }}>{language === 'ru' ? 'Тип' : 'Turi'}:</b>{' '}
                {language === 'ru' ? 'Вопросительные слова о количестве' : 'Savol so\'zlari (sonlar haqida)'}
              </div>
            </div>

            <div className="grammar-block grammar-block--tip">
              <div className="grammar-block__label">{language === 'ru' ? 'Главное отличие' : 'Asosiy farq'}</div>
              <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                <div style={{
                  ...
                  <div style={{ fontSize: 10, color: '#555' }}>
                    {language === 'ru' ? 'Когда ожидается малое число' : 'Kichik son kutilganda'}
                  </div>
                  <div style={{ fontSize: 9, color: '#888', marginTop: 4 }}>
                    {language === 'ru' ? 'Счётное слово НУЖНО' : 'O\'lchov so\'z KERAK'}
                  </div>
                </div>
                <div style={{
                  flex: 1, background: '#fff', borderRadius: 8, padding: 10, textAlign: 'center',
                  ...
                  <div style={{ fontSize: 10, color: '#555' }}>
                    {language === 'ru' ? 'Большое или неизвестное число' : 'Katta yoki noma\'lum son'}
                  </div>
                  <div style={{ fontSize: 9, color: '#888', marginTop: 4 }}>
                    {language === 'ru' ? 'Счётное слово НЕОБЯЗАТЕЛЬНО' : 'O\'lchov so\'z IXTIYORIY'}
                  </div>
                </div>
              </div>
              <p className="grammar-block__tip-note" style={{ textAlign: 'center', marginTop: 10 }}>
                💡 {language === 'ru'
                  ? 'Ответ меньше 10 → 几. Большой или неизвестный → 多少.'
                  : 'Javob 10 dan kichik bo\'lsa → 几. Katta yoki noma\'lum → 多少.'}
              </p>
            </div>

            <div className="grammar-block">
              <div className="grammar-block__label">{language === 'ru' ? 'Что такое счётное слово (量词)?' : 'O\'lchov so\'z (量词) nima?'}</div>
              <div className="grammar-block__tip-text">
                {language === 'ru'
                  ? 'В китайском языке между числом и существительным нужно специальное слово — как «штука», «голова» в русском:'
                  : 'Xitoy tilida son va ot orasiga maxsus so\'z kerak. O\'zbek tilidagi «ta» ga o\'xshash:'}
              </div>
              <div style={{ textAlign: 'center', margin: '8px 0', fontSize: 14 }}>
                <span style={{ color: '#888' }}>{language === 'ru' ? 'три' : 'uch'}</span>
                <span style={{ color: '#f59e0b', fontWeight: 700 }}>{' '}{language === 'ru' ? ' штуки ' : ' ta '}</span>
                <span style={{ color: '#888' }}>{language === 'ru' ? 'книги' : 'kitob'}</span>
                {' = '}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 4, marginTop: 8 }}>
                {[
                  ...
                  <div key={i} style={{
                    ...
                    <div style={{ fontSize: 9, color: '#888' }}>{language === 'ru' ? w.ru : w.uz}</div>
                  </div>
                ))}
              </div>
              <p className="grammar-block__tip-note">
                💡 {language === 'ru' ? '个 (ge) — самое универсальное. Если не знаете — используйте 个!' : '个 (ge) — eng universal o\'lchov so\'z. Bilmasangiz — 个 ishlating!'}
              </p>
            </div>
          </>
        )}
        ...
        {activeTab === 'usage' && (
          <>
            <div className="grammar-block">
              <div className="grammar-block__label">{language === 'ru' ? '1. 几 + счётное слово + существительное' : '1. 几 + O\'lchov so\'z + Ot (kichik son)'}</div>
              <div className="grammar-block__formula">
                <span className="grammar-block__formula-verb" style={{ color: COLOR }}>几</span>
                {' + '}
                <span style={{ color: '#f59e0b', fontWeight: 700 }}>{language === 'ru' ? 'СС' : '量词'}</span>
                {' + '}
                <span className="grammar-block__formula-b">{language === 'ru' ? 'Сущ' : 'Ot'}</span>
                {' ？'}
              </div>
              <p className="grammar-block__formula-desc">
                {language === 'ru' ? 'Когда ожидается ответ меньше 10 (1-9)' : 'Javob 10 dan kichik kutilganda (1-9)'}
              </p>
              {[
              ...
              <div className="grammar-block grammar-block--warning" style={{ marginTop: 8 }}>
                <p className="grammar-block__tip-text">
                  ⚠️ <b>{language === 'ru' ? 'С 几 счётное слово ОБЯЗАТЕЛЬНО!' : '几 bilan o\'lchov so\'z MAJBURIY!'}</b>{' '}
                  {language === 'ru' ? '几人？ нельзя → 几口人？ или 几个人？' : '几人？ emas → 几口人？ yoki 几个人？'}
                </p>
              </div>
            </div>

            <div className="grammar-block">
              <div className="grammar-block__label">{language === 'ru' ? '2. 几 для времени и дат' : '2. 几 vaqt va sana bilan'}</div>
              <p className="grammar-block__tip-text">
                {language === 'ru'
                  ? '几 очень часто используется для вопросов о времени и дате:'
                  : '几 vaqt va sana so\'rashda juda ko\'p ishlatiladi:'}
              </p>
              {[
                ...
                <div key={i} className="grammar-block__usage-item">
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 2 }}>
                    <span className="grammar-block__usage-tr">{language === 'ru' ? x.ru : x.uz}</span>
            ...
            <div className="grammar-block">
              <div className="grammar-block__label">{language === 'ru' ? '3. 多少 + (счётное слово) + существительное' : '3. 多少 + (量词) + Ot (katta son)'}</div>
              <div className="grammar-block__formula">
              ...
              <p className="grammar-block__formula-desc">
                {language === 'ru' ? 'Ответ больше 10 или неизвестен. Счётное слово необязательно.' : 'Javob 10 dan ko\'p yoki noma\'lum. O\'lchov so\'z ixtiyoriy.'}
              </p>
              {[
            ...
            <div className="grammar-block">
              <div className="grammar-block__label">{language === 'ru' ? '4. 多少钱？ = Сколько стоит?' : '4. 多少钱？ = qancha turadi?'}</div>
              <div style={{
                ...
                <div style={{ fontSize: 12, color: '#888', marginTop: 2 }}>
                  {language === 'ru' ? 'Сколько стоит? / Какая цена?' : 'Qancha turadi? / Narxi qancha?'}
                </div>
              </div>
              {[
              ...
              <p className="grammar-block__tip-note">
                💡 <b>多少钱</b> —{' '}
                {language === 'ru'
                  ? 'одна из самых полезных фраз! В магазине, на рынке, в такси — везде нужна.'
                  : 'xitoy tilida eng foydali iboralardan biri! Do\'konda, bozorda, taksida — hamma joyda kerak.'}
              </p>
            </div>

            <div className="grammar-block">
              <div className="grammar-block__label">{language === 'ru' ? '5. 多 + прилагательное = Насколько?' : '5. 多 + Sifat = qanchalik?'}</div>
              <div className="grammar-block__formula">
                <span className="grammar-block__formula-verb" style={{ color: COLOR_DARK }}>多</span>
                {' + '}
                <span className="grammar-block__formula-b">{language === 'ru' ? 'Прил' : 'Sifat'}</span>
                {' ？'}
              </div>
              <p className="grammar-block__formula-desc">
                {language === 'ru' ? 'Насколько высокий? Насколько далеко? и т.д.' : 'Qanchalik baland? Qanchalik uzoq? va h.k.'}
              </p>
              {[
        ...
        {activeTab === 'examples' && (
          <>
            ...
            <div className="grammar-block">
              <div className="grammar-block__label">{language === 'ru' ? 'Диалог: В магазине' : 'Mini dialog 1: Do\'konda'}</div>
            ...
            <div className="grammar-block">
              <div className="grammar-block__label">{language === 'ru' ? 'Диалог: Знакомство' : 'Mini dialog 2: Yangi tanish'}</div>
              <div style={{ background: '#f5f5f8', borderRadius: 8, padding: 12 }}>
                {[
            ...
            <div className="grammar-block">
              <div className="grammar-block__label">{language === 'ru' ? 'Самые частые вопросы' : 'Eng ko\'p ishlatiladigan savollar'}</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
                {[
                  ...
                  <div key={i} style={{
                    ...
                    <div style={{ fontSize: 10, color: '#888' }}>{language === 'ru' ? w.ru : w.uz}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        {activeTab === 'compare' && (
          <>
            <div className="grammar-block">
              <div className="grammar-block__label">{language === 'ru' ? '几 vs 多少 — подробно' : '几 vs 多少 — batafsil'}</div>
              <div style={{ overflowX: 'auto' as const }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                  ...
                  <tbody>
                    {[
                      { ji: language === 'ru' ? '1-9 (малое число)' : '1-9 (kichik son)', duo: language === 'ru' ? '10+ (большое или неизвестное)' : '10+ (katta yoki noma\'lum)' },
                      { ji: language === 'ru' ? 'Счётное слово НУЖНО: 几个、几本' : 'O\'lchov so\'z KERAK: 几个、几本', duo: language === 'ru' ? 'Счётное слово необязательно: 多少(个)人' : 'O\'lchov so\'z ixtiyoriy: 多少(个)人' },
                      { ji: language === 'ru' ? 'Конкретный вопрос (ответ приблизительно известен)' : 'Aniq savol (javob taxminan ma\'lum)', duo: language === 'ru' ? 'Открытый вопрос (ответ неизвестен)' : 'Ochiq savol (javob noma\'lum)' },
            ...
            <div className="grammar-block">
              <div className="grammar-block__label">{language === 'ru' ? 'Одинаковый вопрос — два варианта' : 'Bir xil savol — ikki variant'}</div>
              {[
                {
                  ji: '你有几个朋友？', jiuz: language === 'ru' ? 'Сколько у тебя друзей? (3-5 ожидается)' : 'Nechta do\'sting bor? (3-5 kutilsa)',
                  duo: '你有多少个朋友？', duouz: language === 'ru' ? 'Сколько у тебя друзей? (много ожидается)' : 'Nechta do\'sting bor? (ko\'p kutilsa)',
                },
                {
                  ji: '你买了几本书？', jiuz: language === 'ru' ? 'Сколько книг ты купил? (мало)' : 'Nechta kitob olding? (biroz)',
                  duo: '你买了多少本书？', duouz: language === 'ru' ? 'Сколько книг ты купил? (много)' : 'Nechta kitob olding? (ko\'p)',
                },
                {
                  ji: '教室里有几个人？', jiuz: language === 'ru' ? 'Сколько человек в классе? (мало)' : 'Xonada nechta kishi? (biroz)',
                  duo: '教室里有多少人？', duouz: language === 'ru' ? 'Сколько человек в классе? (много)' : 'Xonada nechta kishi? (ko\'p)',
                },
              ].map((x, i) => (
            ...
            <div className="grammar-block">
              <div className="grammar-block__label">{language === 'ru' ? 'Три способа спросить возраст' : 'Yosh so\'rash — uch usul'}</div>
              <p className="grammar-block__tip-text">
                {language === 'ru'
                  ? 'В китайском вопрос о возрасте меняется в зависимости от возраста собеседника:'
                  : 'Xitoy tilida yosh so\'rash yoshga qarab o\'zgaradi:'}
              </p>
              {[
            ...
            <div className="grammar-block grammar-block--warning">
              <div className="grammar-block__label">{language === 'ru' ? 'Типичные ошибки' : 'Xato qilmaslik uchun'}</div>
              {[
