...
- `FlashcardDeck.tsx` **matches the Arabic deck** (`ar-fc` flip-card UI + 2-grade SRS). Session = cards **due now** + never-seen cards (no row = due immediately; no new-card cap), built **once on mount** via `builtRef`. Front = 汉字 + pinyin + 🔊 audio; flip → translation.
- **Grading**: two buttons — **I don't know** (`dontKnow`) / **I know** (`know`) — plus a **full-width "Move to back of the stack"** button (`.ar-fc__btn--back`) that re-queues the current card within the session (no schedule write). NOT the 3-grade SM-2 anymore. Scheduler is the shared 2-grade `src/lib/arabicSrs.ts` (`dontKnow → 1d`; `know → 4d` first time / after reset, else `×2.5`, cap 365d). `src/lib/srs.ts` (SM-2) is now unused by the deck.
