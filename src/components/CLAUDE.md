...
- **Reader chrome (2026-08-07 rework — see `docs/superpowers/specs/2026-08-07-dialogue-reader-rework.md`)**: once content loads the **hero is hidden** (`.dr-hero--hidden`) and the reader's only top chrome is the **translation bar** (`.dr-trbar`) — 120px, fixed height, full-bleed, rendered **only while Tarjima is on**, a **sibling of `.dr-dialog-body`** (that column is centred `fit-content`, so full-bleed measured from inside it lands off-centre). It measures that column at runtime so its text lines up with the Chinese on both edges, sets its own **absolute 18px** (the A−/A+ control must not scale chrome) and **shrinks to 12px** rather than clipping. Idle it shows a centred hint; translations are left-aligned. **The hero stays on the teaser** — that page is public and its `<h1>` lives there. Back ‹ sits at the left of the bottom tab bar (mirroring ⋮); the ⋮ menu is **Til — Tarjima/Pinyin/Fokus (on/off switches, not checkmarks) — Shrift — Chiqish**. Til is there because the hamburger went with the hero.
- **Per-sentence translation + audio**: about half of all entries hold more than one sentence (1327/3003), so `src/utils/splitSentences.ts` splits an entry and pairs each sentence with its own slice of the translation — **only when both sides yield the same count** (99.9/99.7/99.0% uz/ru/en); otherwise it returns the whole entry, which is the old behaviour. `units` (a memo) is the playback list: **play-all, taps and the TTS prefetch all walk sentences, not entries**. Single-sentence entries keep the entry id as their key so they reuse existing clips. Nothing here has recorded audio — every clip is TTS from text, which is why splitting is free. Tapping also **scrolls the line under the bar** and **narrows the highlight** to the tapped sentence.
- **Preferences persist** via `src/hooks/usePersistedState.ts`: pinyin, translation, font size. Applied in an effect (a lazy initialiser reading localStorage is a hydration mismatch) and range-checked on read. **Focus mode deliberately does not persist.**
- **Pinyin toggling never reflows**: the pinyin row is reserved whether or not pinyin is shown (rows were 70px apart on, 47px off). This also keeps `.dr-line-chars`' `column-gap` alive.
- **Focus mode**: **no translation at all** (Tarjima is disabled + greyed while it is on), left-aligned, `line-height: 2.2`. Full pinyin remains deliberate — see the ⚠️ note below.
...
- **Progressive pinyin** (Dialog tab): when pinyin is on it shows **progressively** — pinyin is **hidden for words BELOW the dialogue's HSK level** (`meta.level`), shown for at-or-above + off-list words. The content API (`/api/content/dialogue/[book]/[slug]` → `attachWordLevels` in `src/lib/hskWordLevels.ts`) attaches a per-character HSK-3.0 level array (`charLvls`) to each sentence; `DialogueReader` hides a char's pinyin when `charLvls[k] < meta.level`. Word boundaries come from the sentence's `words[]` (HSK 1) or CC-CEDICT longest-match segmentation (`content/segwords.txt`, HSK 2+ which ship no word data); a word's level = its HSK headword level, else the **max** of its characters' levels, else off-list. Hidden chars render a **non-breaking-space** placeholder (`.dr-char-py--empty`, `visibility: hidden`) so row heights stay even (a plain space collapses to 0 height).
  - ⚠️ **Focus mode shows FULL pinyin — this is INTENTIONAL, do not "fix" it.** (Since 2026-08-07 it also shows **no translation at all**, which reinforces the same intent: Chinese + full pinyin, nothing else.) Focus mode renders through `RubyText`, which has no `charLvls` awareness, so every word keeps its pinyin. That is the deliberate escape hatch for learners who need full support: the Dialog tab challenges you (progressive), focus mode helps you (full). Effective grid: Dialog+pinyin = progressive · Dialog−pinyin = none · **Focus+pinyin = FULL** · Focus−pinyin = none. It also removes the need for a third "show all" toggle state. Decided 2026-07-21. There is deliberately **no UI affordance** advertising it.
