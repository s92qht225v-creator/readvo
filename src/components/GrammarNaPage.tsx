...
export function GrammarNaPage() {
  ...
  const handleQuizComplete = ({ scores, shadowingUsed }: { scores: import('./SpeakingMashq').Score[]; shadowingUsed: boolean }) => {
    ...
    if (existing === undefined || newStars > existing) saveStars('na', newStars);
  };
            ...
            {examples.map((ex, i) => (
              ...
              >
                <div className="grammar-block__example-zh" style={{display:'flex',alignItems:'center',gap:8}}>
                  <button type="button" className="grammar-play-btn" onPointerDown={e=>{e.stopPropagation();}} onClick={e=>{e.stopPropagation();playGrammarAudio(ex.parts.map((p:{text:string;color:string})=>p.text).join(''));}} style={{background:'#f0f9ff'}} aria-label="Play">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="#0891b2"><path d="M8 5v14l11-7z"/></svg>
                  </button>
            ...
            <div className="grammar-block">
              ...
              {dialog1.map((line, i) => (
                ...
                >
                    ...
                    <span style={{fontWeight:700,color:line.s==='A'?C_NA:C_SHI,flexShrink:0,paddingTop:3}}>{line.s}:</span>
                    <div style={{flex:1}}>
