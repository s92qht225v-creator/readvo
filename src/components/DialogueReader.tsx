          ...
          <>
            ...
            <div className="dr-tabs">
              <div className="dr-tabs__inner">
                {/* The hero carried the only back button; without one here the
                    sole way out is Chiqish, two taps deep inside the ⋮. Sits at
                    the opposite end of the bar from that ⋮. */}
                <Link
                  href={listPath || `${bookPath}/dialogues`}
                  className="dr-tabs__tab dr-tabs__back"
                  aria-label={({ uz: 'Orqaga', ru: 'Назад', en: 'Back' } as Record<string, string>)[language]}
                >
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><polyline points="15 18 9 12 15 6"/></svg>
                </Link>
                {TABS.map(t => (
            ...
            {activeTab === 'dialog' && (
              <>
                {/* The reader's only top chrome — the hero is hidden once
                    ...
                    Only exists while Tarjima is on. With translations off it
                    has nothing to say, and 120px of band repeating the title
                    is worse than the hero it replaced.
