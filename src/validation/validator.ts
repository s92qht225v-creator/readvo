...
export class PageValidator {
  ...
  private validateSection(section: Section, path: string): void {
    ...
    // Section type
    const validTypes = ['objectives', 'text', 'dialogue', 'vocabulary', 'grammar', 'tip', 'exercise', 'instruction', 'activity', 'tonguetwister', 'matching', 'fillblank', 'multiplechoice', 'imagedescribe', 'bonus', 'typedfillblank', 'errorcorrection', 'wordchoice', 'texterror'];
    ...
    if (!section.sentences || section.sentences.length === 0) {
      ...
      if (!section.image_url && !section.context && section.type !== 'matching' && section.type !== 'fillblank' && section.type !== 'multiplechoice' && section.type !== 'imagedescribe' && section.type !== 'bonus' && section.type !== 'tip' && section.type !== 'activity' && section.type !== 'text' && section.type !== 'typedfillblank' && section.type !== 'errorcorrection' && section.type !== 'wordchoice' && section.type !== 'texterror' && section.type !== 'grammar') {
