...
export async function generateMetadata({ params }: PageParams) {
  ...
  const dialogue = await getDialogue('hsk5', dialogueId);
...
export default async function DialoguePage({ params }: PageParams) {
  ...
  const raw = await getDialogue('hsk5', dialogueId);
  ...
  const jsonLd = jsonLdScript([
    breadcrumbJsonLd([
      ...
      { name: dialoguesLabel, path: `/${locale}/chinese?dialhsk=5` },
      { name: `${raw.title} — ${translation}`, path: `/${locale}/chinese/hsk5/dialogues/${dialogueId}` },
    ]),
    {
      '@type': 'LearningResource',
      name: `${raw.title} (${raw.pinyin}) — ${translation}`,
      description: `HSK 5 Chinese dialogue: ${raw.title} — ${translation}`,
  ...
  return (
    <>
      ...
      <DialogueReader
        ...
        bookPath="/chinese/hsk5"
        listPath="/chinese?dialhsk=5"
      />
    </>
  );
}
