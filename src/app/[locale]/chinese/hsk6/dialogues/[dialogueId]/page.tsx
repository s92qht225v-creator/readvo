...
export async function generateMetadata({ params }: PageParams) {
  ...
  const dialogue = await getDialogue('hsk6', dialogueId);
...
export default async function DialoguePage({ params }: PageParams) {
  ...
  const raw = await getDialogue('hsk6', dialogueId);
  ...
  const jsonLd = jsonLdScript([
    breadcrumbJsonLd([
      ...
      { name: dialoguesLabel, path: `/${locale}/chinese?dialhsk=6` },
      { name: `${raw.title} — ${translation}`, path: `/${locale}/chinese/hsk6/dialogues/${dialogueId}` },
    ]),
    {
      '@type': 'LearningResource',
      name: `${raw.title} (${raw.pinyin}) — ${translation}`,
      description: `HSK 6 Chinese dialogue: ${raw.title} — ${translation}`,
  ...
  return (
    <>
      ...
      <DialogueReader
        ...
        bookPath="/chinese/hsk6"
        listPath="/chinese?dialhsk=6"
      />
    </>
  );
}
