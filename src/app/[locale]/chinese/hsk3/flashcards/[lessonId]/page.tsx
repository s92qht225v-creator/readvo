...
import { getWritingSet, WRITING_SETS_HSK3 } from '@/services/writing';
...
export async function generateMetadata({ params }: Props) {
  ...
  const pageMeta: Record<string, { title: string; description: string }> = {
    uz: {
      title: `HSK 3 ${setNum}-to'plam — Fleshkartalar`,
      description: `HSK 3 ${setNum}-to'plam so'zlarini fleshkartalar bilan yodlang.`,
    },
    ru: {
      title: `HSK 3 Набор ${setNum} — Флешкарты`,
      description: `Учите слова HSK 3, набор ${setNum} с флешкартами.`,
    },
    en: {
      title: `HSK 3 Set ${setNum} — Flashcards`,
      description: `Learn HSK 3 set ${setNum} words with flashcards.`,
    },
  };
  ...
  return {
    ...
    alternates: {
      canonical: `/${locale}/chinese/hsk3/flashcards/${lessonId}`,
      languages: {
        uz: `/uz/chinese/hsk3/flashcards/${lessonId}`,
        ru: `/ru/chinese/hsk3/flashcards/${lessonId}`,
        en: `/en/chinese/hsk3/flashcards/${lessonId}`,
        'x-default': `/uz/chinese/hsk3/flashcards/${lessonId}`,
      },
    },
  };
}
...
export async function generateStaticParams() {
  return WRITING_SETS_HSK3.map((s) => ({ lessonId: s.id }));
}

export default async function Hsk3FlashcardsPage({ params }: Props) {
  const { locale, lessonId } = await params;
