...
import { loadArabicStory } from '@/services/arabicContent';
...
export async function GET(
  ...
  const story = loadArabicStory(level, slug);
  if (!story) {
    return NextResponse.json({ error: 'not_found' }, { status: 404 });
  }
  // Returned under the `dialogue` key so the shared ArabicDialogueReader consumes it unchanged.
  return NextResponse.json({ dialogue: story }, { headers: { 'Cache-Control': 'no-store, private' } });
}
