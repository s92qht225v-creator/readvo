...
- `.dr-char-py` - Per-char progressive pinyin (`font-size: 0.69em`, `color: #dc2626`, `margin-bottom: 4px`, `position: relative; top: 6px`). ⚠️ The `top: 6px` is how the pinyin sits low (2026-08-08) — **not** the margin. Measured: the 11px visual gap is only 4px margin; 5.4px is half-leading inside `.dr-char-zh`'s line box and 1.7px the pinyin's, so shrinking the margin is nearly invisible. Don't "simplify" this to a margin, and don't cut `.dr-char-zh`'s `line-height` instead — that strips the same space from BELOW every character and tightens dialogue line spacing.
