...
- `.dr-line-chars` - Char row (flex wrap, `column-gap: 0.18em`, `row-gap: 0.3em`, `padding: 0.07em 0 0.225em`). ⚠️ The row-gap and padding are **load-bearing**: `.dr-char-py`/`.dr-char-zh` use `line-height: 1`, so the half-leading that used to separate wrapped rows and pad the card is gone and is re-stated here. Remove them and wrapped rows collide. ⚠️ The `:not(:has(.dr-char-py))` collapse below it is **dead since 2026-08-07** — the pinyin row is reserved even when pinyin is off, so toggling changes neither line spacing nor character spacing.
- `.dr-char-py` / `.dr-char-zh` - Per-char progressive pinyin (`0.69em`, `#dc2626`) and character (`1.5em`). Both are **`line-height: 1`** so the pinyin↔character gap is exactly one value: `.dr-char-py`'s `margin-bottom: 0.3em` (≈5px at 100%). ⚠️ Keep that gap in **em** — `.dialogue-reader` carries an inline `font-size: 80–150%` from A−/A+ and everything below is em; a px gap drifts against the glyphs (measured 12.6% → 15.9% of glyph height across the range). ⚠️ Don't restore the old `line-height: 1.2/1.3`: that leading was 7 of the gap's 11px and it silently doubled as wrapped-row separation + card padding, which now live on `.dr-line-chars`.
