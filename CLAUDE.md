...
- **Dictionary (M5)** — public, SSG, `/{locale}/chinese/dictionary` (`DictionarySearch.tsx`, `revalidate = 86400`). Searches 汉字 / toneless pinyin ("kongtiao") / native word across `hsk_words` + `glossary` via `GET /api/dictionary` (glossary gloss **wins**; all three glosses ride on every row so switching UI language never refetches). Ranking: exact 汉字 → exact pinyin → prefix → gloss, then **lower HSK level first** (else 医保 outranked 医生). Rows **expand in place** (no navigation — lookups come in bursts): "Add to my vocabulary" (`useSavedVocab`) + example sentences. Word pages not built.
- **Dictionary examples** — `word_examples` table (migration `supabase/migrations/20260727_word_examples.sql`, service-role only), read by `GET /api/dictionary/examples?zh=&py=`. Sentences are **generated per headword and capped at that headword's own HSK level** by `scripts/gen-word-examples.py`. ⚠️ They are deliberately **NOT** mined from the dialogues — that was the first cut and it inverted the difficulty: 白天 is HSK 1 but its only dialogue occurrences are HSK 5/6 (我现在都有点神经衰弱了). Two gates before a sentence is stored: (1) a **level validator** that segments with CC-CEDICT and rejects any word above the headword's level — same rule as `hskWordLevels.ts`, so the dictionary and progressive pinyin can never disagree; (2) a **proofreader pass** that drops invented forms the validator can't see (it caught 今天星期七). Failures are dropped, never downgraded. **Coverage: 20,086 sentences over HSK 1–7, ~92% of headwords, 0 above level** (generated with `gpt-4o-mini`; re-run a level with `EXAMPLES_MODEL=… python3 scripts/gen-word-examples.py --level N`, it upserts). **Off-list words get no examples** — 天天/微信/上海 aren't in HSK 3.0, so there is no level to cap at and the pipeline skips them.
